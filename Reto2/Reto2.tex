...
\algorithm{#1}{$C = \{c_1, c_2, c_3, c_4, c_5, c_6\}$, $T \in [100, 999)$}{Una serie de operaciones con elementos de $C$ y con elementos obtenidos de operaciones previas que se acercan a $T$ lo máximo posible}{#2}
}
...
\algorithm{de obtención de operaciones}{$L$, $T$, con $T = [n, i, j, op]$ o $T = n$}{La serie de operaciones con elementos de $L$ con la que se ha llegado a $n$}{
	...
	\item Si $op$ no es una operación conmutativa (es decir, es resta o cociente) y el valor de $L_i$ es menor que el de $L_j$, intercambiar $i$ y $j$
	\item Devolver la concatenación de lo obtenido en los dos primeros puntos junto con \texttt{a operador b = n}, con $a$ el valor de $L_i$ y $b$ el valor de $L_j$
	\end{itemize}
	}
