...
\begin{enumerate}
\item La multiplicación de sus elementos debe ser mayor o igual que 1000. %¿Seguro?
...
\begin{algorithm}[H]
 ...
 \KwIn{$v_0,..v_n$, vector de n elementos;  $N$, meta, $\operatorname{aproximacion}=v_0$,\;
  $p_0,..,p_k$ booleanos}

 \KwOut{\texttt{true}, si $p_i =\texttt{true} $ $\forall i$, \texttt{false} en otro caso}
 \hspace{0.25cm}	Sean los $b_0,..,b_{m}$ resultado de combinar los $v_i$ y $v_j$ $i \neq j$  \;
 \While{$i<m$ y $\exists j$ tal que $p_j=\textbf{false}$}{
